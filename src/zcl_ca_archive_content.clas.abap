...
*   i n t e r f a c e s
    INTERFACES:
      zif_ca_archive_content.            " !!! Includes IF_WORKFLOW = BI_OBJECT + BI_PERSISTENT
...
*     ZIF_CA_ARCHIVE_CONTENT attributes
      mo_viewer                 FOR zif_ca_archive_content~mo_viewer,
      mo_cvc_ac                 FOR zif_ca_archive_content~mo_cvc_ac,
      mo_sel_options            FOR zif_ca_archive_content~mo_sel_options,
      mt_docs                   FOR zif_ca_archive_content~mt_docs,
      mt_filter_al              FOR zif_ca_archive_content~mt_filter_al,
      ms_filter_dms             FOR zif_ca_archive_content~ms_filter_dms,
      ms_bo_key                 FOR zif_ca_archive_content~ms_bo_key,
      ms_bo_desc                FOR zif_ca_archive_content~ms_bo_desc,
      ms_toacu                  FOR zif_ca_archive_content~ms_toacu,
      mv_count                  FOR zif_ca_archive_content~mv_count,
*     ZIF_CA_ARCHIVE_CONTENT methods
      archive                   FOR zif_ca_archive_content~archive,
      archive_via_dnd_dialog    FOR zif_ca_archive_content~archive_via_dnd_dialog,
      attach                    FOR zif_ca_archive_content~attach,
      attach_docs_2_wi          FOR zif_ca_archive_content~attach_docs_2_wi,
      close_windows             FOR zif_ca_archive_content~close_windows,
      copy_to_other_bo          FOR zif_ca_archive_content~copy_to_other_bo,
      delete                    FOR zif_ca_archive_content~delete,
      display                   FOR zif_ca_archive_content~display,
      free                      FOR zif_ca_archive_content~free,
      get                       FOR zif_ca_archive_content~get,
      get_viewer_default_button FOR zif_ca_archive_content~get_viewer_default_button,
      has_content               FOR zif_ca_archive_content~has_content,
      is_document_already_buffered FOR zif_ca_archive_content~is_document_already_buffered,
      set_al_filter             FOR zif_ca_archive_content~set_al_filter,
      set_content               FOR zif_ca_archive_content~set_content,
      set_dms_filter            FOR zif_ca_archive_content~set_dms_filter.
...
*   i n s t a n c e   m e t h o d s
    METHODS:
      ...
      extract_from_filter
        IMPORTING
          it_dms_filter_range      TYPE zca_tt_dms_filter_ranges
...
CLASS zcl_ca_archive_content IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    mo_cvc_ac = zcl_ca_c_archive_content=>get_instance( ).
  ...
  METHOD extract_from_filter.
    ...
    "-----------------------------------------------------------------*
    mo_cvc_ac->is_dms_filter_field_valid( iv_dms_filter_field_name ).
    ...
    CASE sy-subrc.
      WHEN 0 OR 1.
        ...
        LOOP AT lt_connections REFERENCE INTO DATA(lr_connection).
          IF mv_refresh NE mo_cvc_ac->refresh_opt-refresh_from_db AND
...
*  PERFORM give_me_the_contrep(archivelinktoaom) USING sap_object ar_object archiv_id.
          ...
          CATCH cx_sy_itab_line_not_found.
            TRY.
              ...
              CATCH cx_root INTO DATA(lx_catched).
                MESSAGE lx_catched TYPE zcx_ca_archive_content=>c_msgty_s
                                        DISPLAY LIKE zcx_ca_archive_content=>c_msgty_e.
            ENDTRY.
        ENDTRY.
    ENDTRY.
  ...
  METHOD zif_ca_archive_content~archive.
    "-----------------------------------------------------------------*
    ...
    IF iv_link_immed EQ abap_true.
      ...
      RAISE EVENT new_document_stored
        EXPORTING
          refresh_with_opt = mo_cvc_ac->refresh_opt-add_delta_only
          counter_before   = _number_of_docs_before
          counter_now      = mv_count.
    ENDIF.
  ENDMETHOD.                    "zif_ca_archive_content~archive


  METHOD zif_ca_archive_content~archive_via_dnd_dialog.
    "-----------------------------------------------------------------*
  ...
  ENDMETHOD.                    "zif_ca_archive_content~archive_via_dnd_dialog


  METHOD zif_ca_archive_content~attach.
    "-----------------------------------------------------------------*
    "   Attach docs to business object (curr. for ArchiveLink only!) (this deletes filters!)
    "-----------------------------------------------------------------*
    "Actualize documents of object
    get( iv_refresh = mo_cvc_ac->refresh_opt-refresh_from_db ).
    ...
    get( iv_refresh = iv_refresh ).
  ENDMETHOD.                    "zif_ca_archive_content~attach


  METHOD zif_ca_archive_content~attach_docs_2_wi.
    "-----------------------------------------------------------------*
    "   For WF data binding: Attach archived documents to a work item
    "-----------------------------------------------------------------*
    TRY.
        get( iv_refresh    = mo_cvc_ac->refresh_opt-refresh_from_db
        ...
        LOOP AT mt_docs INTO DATA(lo_document)
                        WHERE table_line->ms_data-is_archived EQ abap_true.
          DATA(ls_document_key) = CORRESPONDING zca_s_image_key( lo_document->ms_data-s_al_conn ).
          APPEND VALUE #( BASE zif_ca_c_wf_bos=>cbo_image
                                    instid = CONV #( ls_document_key ) ) TO rt_adhoc_docs.
        ENDLOOP.

      CATCH zcx_ca_error INTO DATA(lx_catched).
        mo_log->add_n_save_exception( ix_catched = lx_catched
                                      iv_class   = CONV #( c_my_typeid )
                                      iv_method  = 'ATTACH_DOCS_2_WI' ) ##no_text.
        mo_log->save( ).
    ENDTRY.
  ENDMETHOD.                    "zif_ca_archive_content~attach_docs_2_wi


  METHOD zif_ca_archive_content~close_windows.
    "-----------------------------------------------------------------*
    ...
    TRY.
      ...
      CATCH cx_dv_exception INTO DATA(lx_error).
        ...
        ENDIF.
    ENDTRY.
  ENDMETHOD.                    "zif_ca_archive_content~close_windows


  METHOD zif_ca_archive_content~copy_to_other_bo.
    "-----------------------------------------------------------------*
  ...
  ENDMETHOD.                    "zif_ca_archive_content~copy_to_other_bo


  METHOD zif_ca_archive_content~delete.
    "-----------------------------------------------------------------*
    "   Delete connections (not the docs themselves!)
    "-----------------------------------------------------------------*
    "Local data definitions
    DATA:
      lx_error   TYPE REF TO zcx_ca_archive_content.
    ...
    get( iv_refresh    = abap_true
         it_filter_al  = mt_filter_al
         is_filter_dms = ms_filter_dms ).
  ENDMETHOD.                    "zif_ca_archive_content~delete


  METHOD zif_ca_archive_content~display.
    "-----------------------------------------------------------------*
    "   Display document(s)
    "-----------------------------------------------------------------*
    "Local data definitions
    ...
    TRY.
      ...
      CATCH cx_dv_exception INTO DATA(lx_error).
        ...
        ENDIF.
    ENDTRY.
  ENDMETHOD.                    "zif_ca_archive_content~display


  METHOD zif_ca_archive_content~free.
    "-----------------------------------------------------------------*
    ...
    ENDLOOP.
    FREE mt_docs.
  ENDMETHOD.                    "zif_ca_archive_content~free


  METHOD zif_ca_archive_content~get.
    "-----------------------------------------------------------------*
    ...
    ELSE.
      DATA(_number_of_docs_before) = mv_count.
      IF mv_refresh EQ mo_cvc_ac->refresh_opt-refresh_from_db.
        free( ).
      ENDIF.
      ...
      ENDIF.
    ENDIF.
  ENDMETHOD.                    "zif_ca_archive_content~get


  METHOD zif_ca_archive_content~get_viewer_default_button.
    "-----------------------------------------------------------------*
    "   Get default button of archive content viewer
    "-----------------------------------------------------------------*
    result = cl_dv_viewer=>get_default_toolbar_buttons( ).
  ENDMETHOD.                    "zif_ca_archive_content~get_viewer_default_button


  METHOD zif_ca_archive_content~has_content.
    "-----------------------------------------------------------------*
  ...
  METHOD zif_ca_archive_content~is_document_already_buffered.
    "-----------------------------------------------------------------*
    "   Check whether the document is already buffered
    "-----------------------------------------------------------------*
    result = xsdbool( mv_refresh NE mo_cvc_ac->refresh_opt-refresh_from_db AND
                      line_exists( mt_docs[ table_line->ms_data-arc_doc_id = iv_file_id ] ) ).
  ENDMETHOD.                    "zif_ca_archive_content~is_document_already_buffered


  METHOD zif_ca_archive_content~set_al_filter.
    "-----------------------------------------------------------------*
    "   Set filter for ArchiveLink documents
    "-----------------------------------------------------------------*
    mt_filter_al = it_filter_al.
  ENDMETHOD.                    "zif_ca_archive_content~set_al_filter


  METHOD zif_ca_archive_content~set_content.
    "-----------------------------------------------------------------*
    "   Set content, e. g. after manipulating the result in MT_DOCS
    "-----------------------------------------------------------------*
    mt_docs = it_docs.
  ENDMETHOD.                    "zif_ca_archive_content~set_content


  METHOD zif_ca_archive_content~set_dms_filter.
    "-----------------------------------------------------------------*
    "   Set filter for DMS documents
    "-----------------------------------------------------------------*
    ms_filter_dms = is_filter_dms.
  ENDMETHOD.                    "zif_ca_archive_content~set_dms_filter
